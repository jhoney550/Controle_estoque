...
function usuarioCadastroEhAdmin(usuario){
  return String(usuario?.user || '').trim().toLowerCase() === 'admin';
}

function usuarioPodeExcluir(usuario){
  return !!usuario && usuarioLogadoAdmin() && !usuarioCadastroEhAdmin(usuario);
}
...
function uSelect(id){
  ...
  document.getElementById('u-btn-alt').disabled = !usuarioLogadoAdmin() || usuarioCadastroEhAdmin(u);
  document.getElementById('u-btn-exc').disabled = !usuarioPodeExcluir(u);
...
async function uExcluir(){
  ...
  if(usuarioCadastroEhAdmin(u)){
    toast('O usuario admin nao pode ser excluido!', 'danger');
    return;
  }
  ...
  if(!confirm(`Excluir o usuario ${u.user}?`)) return;
  ...
  if(error){
    console.error(error);
    toast('Erro ao excluir usuario.', 'danger');
    return;
  }
...
function uResetBtns(){
  ...
  document.getElementById('u-btn-alt').disabled =
    !uState.sel || !usuarioLogadoAdmin() || usuarioCadastroEhAdmin(u);

  document.getElementById('u-btn-exc').disabled = !uState.sel || !usuarioPodeExcluir(u);
  document.getElementById('u-btn-grav').disabled = true;
