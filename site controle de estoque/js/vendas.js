...
const camposAuditoriaVenda = [
  ...
  ['obs', 'observação'],
  ['cancelada', 'cancelada', v => vendaCancelada({cancelada:v}) ? 'Sim' : 'Não'],
...
function abrirDedoDuroVenda(id){
  ...
  abrirDedoDuroGenerico('vendas', id, `Dedo Duro - Venda Nº ${v?v.nrv:id}`);
}
...
function vValidarEstoqueQuantidade(showMsg=true){
  ...
  if(disp<qtd){
    ...
    btn.disabled=true;
    if(showMsg)toast(`Estoque insuficiente! Disponível: ${disp} un.`,'danger');
    return false;
  }
  qtdInput.classList.remove('is-invalid');
  if(vState.mode)btn.disabled=false;
...
function vAddItem(){
  ...
  if(unit<=0){toast('Informe o valor unitário!','danger');return;}
  
  if(!vValidarEstoqueQuantidade(true))return;
  const stock=getDisponivelVenda(pid, vState.itemEditIdx);
  if(stock<qtd){toast(`Estoque insuficiente! Disponível: ${stock} un.`,'danger');return;}
...
function renderCart(){
  ...
  // LINHA 554 (Final da função renderCart)
...
if (desconto > subtotalVenda) {
  toast('O desconto não pode ser maior que o total da venda!', 'danger');
...
if (vState.mode === 'incluir' && vendasCache.some(venda => venda.nrv === vState.nrv)) {
  toast('Esta venda já foi finalizada!', 'warning');
  return;
}
...
registrarAuditoriaGenerica(
  ...
  // Avança o contador interno de ID/Vendas global do sistema
  ...
  // LIMPEZA DA TELA IMEDIATA (Reseta os campos do formulário para o operador)
  vState.mode = null;
  vState.itens = [];
  vState.nrv = null;
  vState.editId = null;
...
function showImp(v){
  ...
  const cancelada=vendaCancelada(v);
  statusText.textContent=cancelada?'Venda cancelada':'Venda concluída com sucesso!';
...
async function verVenda(id){
  ...
  if (!v) {
    toast('Venda não encontrada!', 'danger');
    return;
  }
...
async function confirmarVendaAdmin(){
  ...
  if(!admin || String(admin.user).toLowerCase() !== 'admin'){
    toast(
      action === 'editar'
        ? 'Alteração não permitida, contate o administrador'
        : 'Cancelamento não permitido. contate o administrador',
      'danger'
    );
    return;
  }
...
async function editarVenda(id){
  ...
  if (vendaCancelada(v)) {
    toast('Venda cancelada não pode ser alterada.', 'warning');
    return;
  }

  if (vState.itens.length && !confirm('A venda atual será substituída pela venda selecionada. Continuar?')) {
    return;
  }
  ...
  toast('Venda carregada para edição.', 'info');
}
...
async function cancelarVenda(id, obs, admin){
  ...
  if(vendaCancelada(v)){
    toast('Venda já está cancelada.', 'warning');
    return;
  }

  if (!confirm(`Cancelar a venda Nº ${v.nrv}?`)) return;
...
function doPrint(){
  // Imprime de forma limpa disparando a janela nativa de impressão do Windows
  window.print();
}
...
function fecharImp() {
  ...
  // Limpa completamente o estado do PDV para a próxima venda
  vState.mode = null;
  vState.itens = [];
  vState.nrv = null;
  vState.editId = null;
  ...
  // Limpa controles padrão do formulário do PDV
  document.getElementById('v-btn-exc').disabled = true;
  document.getElementById('v-btn-fin').disabled = true;
  document.getElementById('v-btn-can').style.display = 'none';
  document.getElementById('v-btn-add').disabled = true;
  ...
  // renderiza o carrinho vazio e atualiza o histórico e relatório de vendas na hora
  renderCart();
  loadVHist();
  const inicio = document.getElementById('section-inicio');
  if (inicio && inicio.classList.contains('active')) {
...
function vCancelar(){
  if(vState.itens.length&&!confirm('Cancelar a venda? Os itens serão removidos.'))return;
...
function vProdSearch(){
  ...
  dd.innerHTML=ps.map(p=>{
    ...
    return `<div class="dd-item" onclick="vProdSelect('${p.id}','${p.nome.replace(/'/g,"\\'")}')">${p.id} — ${p.nome} <span class="badge ${st>0?'bg-success':'bg-danger'}">${st} un.</span></div>`;
  }).join('');
  dd.style.display='';
}
